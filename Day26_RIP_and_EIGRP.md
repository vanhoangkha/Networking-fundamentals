...
By Mặc định, RIP-Enabled ROUTERS will share their Định tuyến TABLE every 30 seconds
...
- Doesn’t include Mạng con MASK information in ADVERTISEMENTS (RESPONSE messages)
    - Example:
...
- Supports VLSM, CIDR
- Includes Mạng con MASK information in ADVERTISEMENTS
- Messages are **Multicast** to 224.0.0.9
    - Broadcast Messages are delivered to ALL devices on the local Mạng
...
The **“Mạng”** Lệnh tells the Router to:
...
- Advertise the **Mạng PREFIX of the Giao diện** (NOT the prefix in the “Mạng” Lệnh)

The OSPF and EIGRP **“Mạng”** commands operate in the same way

Because the RIP “Mạng” Lệnh is CLASSFUL. It will automatically convert to CLASSFUL networks
...
- Because the “Mạng” Lệnh is CLASSFUL, 172.16.0.0 is assumed to be 172.16.0.0/16
...
- There are NO RIP neighbors connected to G2/0 so no NEW ADJACENCIES are formed
    ...
    - This is unnecessary traffic, so G2/0 should be configured as a **passive Giao diện**

![image](https://github.com/psaumur/CCNA/assets/106411237/634f4c6b-291c-4a21-8ae2-c8283044efce)

- the “passive-Giao diện” Lệnh tells the Router to stop sending RIP advertisements out of the specified Giao diện (G2/0)
- However, the Router will continue to ADVERTISE the Mạng prefix of the Giao diện (172.16.1.0/28) to it’s RIP neighbors (R2, R3)
- You should ALWAYS use this Lệnh on INTERFACES which don’t have any RIP neighbors
- EIGRP and OSPF both have the same passive Giao diện functionality, using the same Lệnh.

---

Cách ADVERTISE A Mặc định Tuyến đường INTO RIP
...
To SHARE this Mặc định Tuyến đường with R1’s RIP neighbors, using this Lệnh:

![image](https://github.com/psaumur/CCNA/assets/106411237/799d818a-06cc-4f29-8c74-c67639c9d014)

RIP doesn’t care about Giao diện AD cost (RIP cost is 120), only “hops”.

Since both have an equal number of “hops”, both paths appear in the Mặc định Tuyến đường (Gateway of Last Resort)
...
“Maximum path: 4” is the Mặc định but can be changed with this Lệnh:

![image](https://github.com/psaumur/CCNA/assets/106411237/35d524bd-055d-4c5e-a84b-f507a87738e0)

“Distance” (AD) can be changed with this Lệnh (Mặc định is 120)
...
- Much faster than RIP in reacting to changes in the Mạng
- Does NOT have the 15 ‘hop count’ limit of RIP
- Sends messages using Multicast ADDRESS **224.0.0.10 (Memorize this number)**
- Is the ONLY IGP that can perform **unequal**-cost load-balancing (by Mặc định, it performs ECMP load-balancing over 4 paths like RIP)
...
- Auto-summary might be ENABLED or DISABLED by Mặc định; depending on the Router/IOS version. If ENABLED, Vô hiệu hóa it.
- The **“Mạng”** Lệnh will assume a CLASSFUL ADDRESS, if you don’t specify the Mạng con MASK
- EIGRP uses a *wildcard mask* instead of a regular Mạng con mask

A WILDCARD MASK is an “inverted” Mạng con MASK

- All 1’s in the Mạng con MASK are 0 in the equivalent WILDCARD MASK.
- All 0s in the Mạng con MASK are 1 in the equivalent WILDCARD MASK.
...
- Manual Cấu hình
- Highest Địa chỉ IP on a LOOPBACK Giao diện
- Highest Địa chỉ IP on a PHYSICAL Giao diện
...
- By Mặc định, EIGRP uses BANDWIDTH and DELAY to calculate Metric
- Mặc định “K” values are:
...
“maximum Metric variance 1” = the Mặc định value
