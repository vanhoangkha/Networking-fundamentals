...
- Có BA PHIÊN BẢN:
- RIPv1 và RIPv2; được sử dụng cho IPv4
- RIPng (RIP Next Generation) được sử dụng cho IPv6
- Sử dụng HAI LOẠI THÔNG ĐIỆP:
- **REQUEST**: Để yêu cầu các ROUTERS láng giềng có RIP gửi BẢNG ĐỊNH TUYẾN của chúng
...
- Ví dụ:
- 10.1.1.0/24 sẽ trở thành 10.0.0.0 (Địa chỉ Class A, nên được giả định là /8)
...
Lệnh **"network"** yêu cầu Router:
- Tìm kiếm CÁC GIAO DIỆN với Địa chỉ IP nằm trong PHẠM VI cụ thể
...
Vì lệnh "network" của RIP là CLASSFUL, nó sẽ tự động chuyển đổi thành mạng CLASSFUL:
...
- R1 QUẢNG CÁO 10.0.12.0/30 và 10.0.13.0/30 (KHÔNG phải 10.0.0.0/8) đến các láng giềng RIP của nó
...
- Đây là traffic không cần thiết, nên G2/0 nên được cấu hình như một **giao diện passive**
...
## 26.4 CÁCH QUẢNG CÁO TUYẾN ĐƯỜNG MẶC ĐỊNH VÀO RIP
...
Để CHIA SẺ Tuyến đường Mặc định này với các láng giềng RIP của R1, sử dụng lệnh này:

![image](https://github.com/psaumur/CCNA/assets/106411237/799d818a-06cc-4f29-8c74-c67639c9d014)

RIP không quan tâm đến chi phí AD của Giao diện (chi phí RIP là 120), chỉ quan tâm "hops". Vì cả hai đều có số "hops" bằng nhau, cả hai đường đi đều xuất hiện trong Tuyến đường Mặc định (Gateway of Last Resort)

![image](https://github.com/psaumur/CCNA/assets/106411237/1deccb54-02e0-4d3b-b203-277d656504b3)

- --

## 26.5 "show ip protocols" (cho RIP)
![image](https://github.com/psaumur/CCNA/assets/106411237/b7ab4046-b6eb-4e19-b7eb-2c5d2889293a)

"Maximum path: 4" là giá trị Mặc định nhưng có thể được thay đổi bằng lệnh này:
...
- Được coi là một GIAO THỨC ĐỊNH TUYẾN DISTANCE VECTOR "nâng cao" / "lai"
...
- Auto-summary có thể được BẬT hoặc TẮT theo Mặc định; tùy thuộc vào phiên bản Router/IOS. Nếu được BẬT, hãy Vô hiệu hóa nó.
- Lệnh **"network"** sẽ giả định một Địa chỉ CLASSFUL, nếu bạn không chỉ định MASK Mạng con
- EIGRP sử dụng *wildcard mask* thay vì mask Mạng con thông thường

**WILDCARD MASK** là một MASK Mạng con "đảo ngược":
- Tất cả số 1 trong MASK Mạng con là 0 trong WILDCARD MASK tương đương
- Tất cả số 0 trong MASK Mạng con là 1 trong WILDCARD MASK tương đương

![image](https://github.com/psaumur/CCNA/assets/106411237/f64e06d3-75ad-4f4f-b7d6-26f27ffae541)

"0" trong WILDCARD MASK = CÁC BIT phải KHỚP!
"1" trong WILDCARD MASK = Không cần khớp
...
## 26.8 "show ip protocols" (cho EIGRP)
...
- Địa chỉ IP Cao nhất trên GIAO DIỆN VẬT LÝ
...
GHI NHỚ NHỮNG GIÁ TRỊ NÀY!

**"show ip route" (cho EIGRP)**

![image](https://github.com/psaumur/CCNA/assets/106411237/8216ceb6-0d3f-42e7-8e5b-46e810097fb8)

LƯU Ý các số Metric lớn. Đây là một NHƯỢC ĐIỂM của EIGRP - ngay cả trên các mạng nhỏ!
...
- **Reported Distance** (còn gọi là **Advertised Distance**) = Giá trị Metric của láng giềng đến ĐÍCH của tuyến đường
...
**ĐIỀU KIỆN KHẢ THI**: Một tuyến đường được coi là ***Feasible Successor*** nếu ***Reported Distance*** của nó THẤP HƠN ***Feasible distance*** của tuyến đường Successor

![image](https://github.com/psaumur/CCNA/assets/106411237/206db633-3a7e-4d11-bb80-029ea8107503)

- --

## 26.11 EIGRP: CÂN BẰNG TẢI CHI PHÍ KHÔNG BẰNG NHAU
...
Variance 1 = chỉ cân bằng tải ECMP (Equal-Cost Multiple Path) sẽ được thực hiện

![image](https://github.com/psaumur/CCNA/assets/106411237/824dac1d-38dc-4e7e-bb48-b382918230ff)

Variance 2 = các tuyến đường ***feasible successor*** với FD lên đến 2x FD của tuyến đường ***successor*** có thể được sử dụng để cân bằng tải
