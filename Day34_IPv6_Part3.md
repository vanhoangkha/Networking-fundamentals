...
- Used for QoS (Chất lượng dịch vụ) to indicate high-priority traffic
...
- Indicates the LENGTH of the PAYLOAD (the Đóng gói LAYER 4 Đoạn) **in bytes**
- The length of the IPv6 Header, itself, isn’t included, because it’s ALWAYS 40 bytes

Next Header (8 bits)

- Indicates the TYPE of the ‘next Header’ (Header of the encapsulated Đoạn)
    - Example: TCP or UDP
- Same function as the IPv4 Header’s ‘Giao thức’ field

Hop Limit (8 bits)

- Value in this field decrements by 1 every time a Router forwards it. If it reaches ‘0’, the Gói tin is discarded (similar to IPv4 TTL field )

Source Address (128 bits)

- Gói tin’s SOURCE address

Destination Address (128 bits)

- Gói tin’s DESTINATION address
...
Cách generate a SOLICITED-NODE Multicast Address
...
NEIGHBOR DISCOVERY Giao thức (NDP)

- NEIGHBOR DISCOVERY Giao thức (NDP) is a Giao thức used with IPv6
- It has various functions and one of those functions is to replace ARP, which is no longer used in IPv6
- The ARP-like function of NDP uses ICMPv6 and SOLICITED-MODE Multicast Addresses to learn the Địa chỉ MAC of other HOSTS (ARP in IPv4 uses Broadcast Messages)
...
[AD] = Khoảng cách quản trị. You NEED this value in order to configure a STATIC Tuyến đường
...
This is a Tuyến đường from R2 to PC1 and PC2 using the “next hop” ADDRESSES of R1 and R3 G0/0 INTERFACES
