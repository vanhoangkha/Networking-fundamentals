...
- All DEVICES on the WLAN will use the same Giao thức, however each CLIENT will use a unique ENCRYPTION / DECRYPTION KEY so that other DEVICES can’t read its traffic
...
- WEP (Wired Equivalent Privacy)
    ...
    - WEP is a “SHARED-KEY” Giao thức, requiring the SENDER and RECEIVER to have the same KEY
...
EAP (Extensible Authentication Giao thức)
...
- PEAP (PROTECTED EAP)
    ...
    - Because only the SERVER provides a CERTIFICATE for AUTHENTICATION, the CLIENT must still be AUTHENTICATED within the SECURE TUNNEL
        - Example: MS-CHAP (Microsoft Challenge-Handshake Authentication Giao thức)
...
- TKIP (Temporal Key Integrity Giao thức)
    ...
    - TKIP adds various Bảo mật FEATURES:
        ...
        - The MIC includes the SENDER Địa chỉ MAC to identify the Khung’s SENDER
        - A TIMESTAMP is added to the MIC to prevent replay attacks. Replay attacks involved re-resending a Khung that has already been transmitted
        - A TKIP SEQUENCE NUMBER is used to keep track of FRAMES sent from each SOURCE Địa chỉ MAC. This also protects against REPLAY ATTACKS
...
- CCMP (Counter / CBC-MAC Giao thức)
    ...
    - CCMP consists of TWO DIFFERENT ALGORITHMS to provide ENCRYPTION and MIC :
        - AES (Advanced Encryption Standard) COUNTER MODE ENCRYPTION
            - AES is the MOST SECURE ENCRYPTION Giao thức currently available.
...
- GCMP (GALOIS / COUNTER MODE Giao thức)
...
- The WI-FI Alliance has developed THREE WPA CERTIFICATIONS for Không dây DEVICES:
    - WPA
    - WPA2
    - WPA3
...
- All of the above support TWO AUTHENTICATION MODES:
    ...
    - Was released in 2018 and includes the following PROTOCOLS:
        ...
        - WPA3 also provides several additional Bảo mật features:
            ...
            - FORWARD SECRECY
                - Prevents DATA from being DECRYPTED after it has been transmitted over the air so an ATTACKER can’t capture Không dây FRAMES and then try to DECRYPT them later
