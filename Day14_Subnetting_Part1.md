...
HOWEVER, only Class A, B, C Addresses can be assigned to a device as an Địa chỉ IP.
...
The problem with 'CLASSFUL' assignment is that it led to Địa chỉ IP wastefulness.
...
CIDR (Classless Inter-Domain Định tuyến) removed the requirements of CLASS A, B, and C regarding size.
